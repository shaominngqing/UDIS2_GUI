...
| 版本 | 文件名 | 主要改进 | 核心特性          |
|------|--------|----------|---------------|
| 1.0 | `gui.py` | 基础功能实现 | 单页面操作，同步执行任务  |
| 2.0 | `gui2.py` | 界面分层优化 | 分页布局，图片实时预览   |
| 3.0 | `gui3.py` | 多线程支持 | 异步任务处理，增强日志   |
| 4.0 | `gui4.py` | 流程整合 | 一键式操作，简化代码结构  |
| 5.0 | `gui5.py` | 中间结果展示 | 全流程可视化、展示中间产物 |
...
>1. 登陆： 使用ssh连接服务器，用户输入账户类似于：ssh -p 31041 root@connect.cqa1.seetacloud.com， 密码类似于： abcd 点击登录后开始连接服务器，连接成功后跳转下一步，失败给出提示
...
## 总结
- **演进趋势**：从基础功能到全流程可视化，逐步解决界面卡顿、交互简陋、调试困难等问题。
- **技术亮点**：多线程、模块化设计、中间结果展示。
- **后续建议**：支持自定义服务器路径、增加进度条、优化线程资源管理。